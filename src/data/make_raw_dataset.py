...
def main(name, map_filter, sample_count, validation_size):
    ...
    while len(contents) < sample_count:
        sample_idx = random.randint(0, len(maps) - 1)
        sample = maps[sample_idx]
        if sample.set_id in contents: continue

        sample_type = 'train' if sample_idx not in validation_samples else 'validation'
        ...
        progress.set_postfix(sample=sample.folder_name, refresh=False)
        progress.update(1)
