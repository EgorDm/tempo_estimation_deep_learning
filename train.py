...
@click.option('--buffer_size', default=60000, type=click.INT, help='Amount of batches that should be preloaded')
...
@click.option('--validation_buffer_size', default=5000, type=click.INT, help='Amount of batches to preload for validation')
def main(model_name, dataset_name, save_name, epochs, samples_per_epoch, batch_size, buffer_size, validation_steps, validation_batch_size,
         validation_buffer_size):
    train_model = importlib.import_module(f'src.models.{model_name}')
    train_model.train(dataset_name, save_name, samples_per_epoch, validation_steps, epochs, batch_size, buffer_size, validation_batch_size,
                      validation_buffer_size)
